...
public class AdminUserManagementPopupController extends BasePopup<User> {
    ...
    public void onBanButton(){
        if(!user.getStatus()) {
            ...
            if(AlertService.showConfirmation("คุณต้องการระงับสิทธิ์" + System.lineSeparator() + user.getNameTitle() + user.getName() + " " + user.getSurname())){
    ...
    public void onUnbanButton(){
        if(user.getStatus()) {
            ...
            if(AlertService.showConfirmation("คุณต้องการคืนสิทธิ์ให้" + System.lineSeparator() + user.getNameTitle() + user.getName() + " " + user.getSurname())){
                user.setStatus(true);
                this.issueEvent("success");
                this.close();
            }
