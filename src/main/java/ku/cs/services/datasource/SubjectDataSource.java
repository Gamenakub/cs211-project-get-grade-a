...
public class SubjectDataSource implements Writable<CourseList, Course>, Readable<CourseList, Course> {

    @Override
    public String getFileName() {
        return "course_data.csv"; // Example file name
    }
    ...
    public Course hashMapToModel(HashMap<String, String> row) {
        //Advisor teacher = advisorList.findAdvisorByName(row.get()); // Assuming Advisor can be constructed with name
        return new Course(
                row.get("courseName"),
                row.get("courseId"),
                row.get("courseYear"),
